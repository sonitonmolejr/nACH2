...
File.prototype.generateBatches = function(done1) {
	...
	async.each(this._batches, function(batch, done2) {	
		...
		async.each(batch._entries, function(entry, done3) {
			entry.fields.traceNumber.value = ( entry.fields.traceNumber.value) ?  entry.fields.traceNumber.value : self.header.immediateDestination.value.slice(0,8) + utils.pad(addendaCount, 7, false, '0');
        ...
        self.control.blockCount.value = utils.getNextMultiple(rows + addendaCount, 10) / 10;
